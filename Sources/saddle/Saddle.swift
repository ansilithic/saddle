...
struct Saddle: AsyncParsableCommand {
    ...
    func run() async {
        ...
        let (allRepos, _, hostResult) = await gatherRepos(
            manifest: manifest,
            devDir: devDir,
            declaredURLs: declaredURLs,
            spinner: spinner,
            fetch: shouldFetch,
            forceHost: forceFetch
        )
    ...
    private func gatherRepos(manifest: Manifest?, devDir: String, declaredURLs: [String], spinner: ProgressSpinner, fetch: Bool, forceHost: Bool = false) async -> (repos: [RepoInfo], missingURLs: [String], host: HostResult) {
        ...
        // Phase 2: fetch all remotes (network). Gated by a 24h cache
        // (State.shouldFetch); --force-fetch bypasses.
        if fetch {
